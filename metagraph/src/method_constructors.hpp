...
std::unique_ptr<BinaryMatrix> matrix_type_to_data(const std::string& file, MatrixType type) {
...
UniquePtrs<bit_vector> convert_to(UniquePtrs<bit_vector>&& input) {
    ...
    for (auto& vector : input) {
        result.emplace_back(new BitVector(vector->convert_to<BitVector>()));
        vector.reset();
    }
    return result;
}
...
generate_brwt_from_rows(std::vector<std::unique_ptr<bit_vector>>&& columns,
    ...
    if (greedy) {
        ...
        // );
        std::cout << "Yes, greedy" << std::endl;
        binary_matrix = std::make_unique<BRWT>(
                BRWTBottomUpBuilder::build(std::move(columns), [](const auto& columns) {

                    // Generate subvectors using random_submatrix
                    std::vector<sdsl::bit_vector> subvectors
                            = random_submatrix(columns, 1'000'000);

                    // Print the intermediate subvectors
                    std::cout << "Generated Subvectors:" << std::endl;
                    ...
                    // Perform greedy_matching on the subvectors
                    auto result = greedy_matching(subvectors);

                    // result is std::vector<std::vector<uint64_t>> Partition
                    // Print the result of greedy_matching
                    std::cout << "Result of greedy_matching:" << std::endl;
        ...
        binary_matrix = std::make_unique<BRWT>(BRWTBottomUpBuilder::build(
                std::move(columns), BRWTBottomUpBuilder::get_basic_partitioner(arity)));
    }
...
generate_from_rows(std::vector<std::unique_ptr<bit_vector>>&& columns,
    ...
    switch (type) {
        ...
        case MatrixType::COLUMN: {
            binary_matrix.reset(
                    new ColumnMajor(convert_to<bit_vector_sd>(std::move(columns))));
            break;
        }
        case MatrixType::BRWT: {
            binary_matrix.reset(new BRWT(BRWTBottomUpBuilder::build(std::move(columns))));
            break;
        }
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic)
    for (uint64_t i = 0; i < columns.size(); ++i) {
        ...
        selector->call_ones([&](uint64_t pos) {
