...
greedyAgglomerativeClusteringHamming(const std::vector<std::unique_ptr<bit_vector>>& data,
    ...
    std::iota(pointsAssignCluster.begin(), pointsAssignCluster.end(),
              0); // Initially, each point is its own cluster
    std::cout << "Initialized pointsAssignCluster" << std::endl;

    std::vector<std::unique_ptr<bit_vector>>
            representatives; // Each cluster initially represented by its own point
    ...
    for (size_t i = 0; i < nPoints; ++i) {
        clustersPoints[i]
                = { static_cast<size_t>(i) }; // Each cluster starts with its own point
...
#pragma omp parallel
        {
            // Thread-local copies of minDistance and bestPair
            int localMinDistance = INT_MAX;
            std::pair<int, int> localBestPair = { -1, -1 };
...
#pragma omp critical
        ...
        for (size_t i = 0; i < representatives[c1]->size(); ++i) {
            (*representatives[c1])[i] = (*representatives[c1])[i]
                    & (*representatives[c2])[i]; // Bitwise AND
        }
...
int main() {
    try {
        ...
        if (argc != 4) {
            std::cerr << "Usage: " << argv[0] << " <folderPath> <numClusters> <samplingSize>" << std::endl;
            return 1;
        }

        std::string folder_path = argv[1];     // Path to the folder containing binary files
        int numClusters = std::stoi(argv[2]);  // Desired number of clusters
        int samplingSize = std::stoi(argv[3]); // Number of random cluster pairs to sample
        ...
        auto [anchors, clusters_points]
                = greedyAgglomerativeClusteringHamming(transposed, numClusters, samplingSize);
