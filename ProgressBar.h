...
private:
    ...
    int updateFrequency; // Frequency of updates
...
public:
    ...
    ProgressBar(int width, const std::string& title, int total, int updateFrequency = 5000)
        : width(width), title(title), total(total), current(0),
          updateFrequency(updateFrequency), startTime(std::chrono::steady_clock::now()) {}

    // Increment the progress bar by 1
    void update() {
        if (current < total) {
            ++current;
            if (current == 1 || current % updateFrequency == 0 || current % (total / width) == 0 || current == total) { // Update less frequently
                render();
            }
        }
    }
